import { ShoppingCart, ArrowLeft, FileText, Plus, X, User, CreditCard, TrendingUp } from "lucide-react";
import { Link } from "react-router-dom";
import { useState, useEffect } from "react";
import { createVente, getAllFactures, getAllProduits } from "../../../apiClient";
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-gray-100 dark:border-slate-800 overflow-hidden transition-colors">
          <div className="p-6 md:p-8 border-b dark:border-slate-800 bg-gray-50/50 dark:bg-slate-800/20 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
            ...
            <div className="relative">
              ...
              {exportMenuOpen && (
                <div className="absolute right-0 mt-3 w-48 bg-white dark:bg-slate-800 border dark:border-slate-700 rounded-2xl shadow-xl z-20 p-2 animate-slide-up">
                  <button
          ...
          {loading ? (
            <div className="py-24 flex flex-col items-center text-center">
              <div className="w-20 h-20 bg-gray-100 dark:bg-slate-800 rounded-full flex items-center justify-center mb-6 animate-pulse">
                <FileText size={36} className="text-gray-400 dark:text-slate-600" />
              </div>
              <h3 className="text-xl font-bold text-gray-900 dark:text-white uppercase tracking-tight">{t("Loading_Invoices")}</h3>
            ...
            <div className="py-24 flex flex-col items-center text-center">
              ...
              <h3 className="text-xl font-bold text-gray-900 dark:text-white uppercase tracking-tight">{t("No_Invoices_Recorded")}</h3>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-gray-50/50 dark:bg-slate-800/50 border-b border-gray-100 dark:border-slate-800 text-[10px] font-black uppercase tracking-widest text-gray-500 dark:text-slate-500">
                    <th className="px-8 py-5">{t("Invoice_No")}</th>
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                  {factures.map((f) => (
                    <tr key={f.idFacture} className="hover:bg-gray-50/80 dark:hover:bg-slate-800/30 transition-colors group">
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-black text-gray-900 dark:text-white text-sm uppercase tracking-tight">{f.numeroFacture}</span>
                        </div>
                      </td>
                      <td className="px-6 py-5 text-sm font-medium text-gray-600 dark:text-slate-300">
                        {new Date(f.dateFacture).toLocaleString()}
                      </td>
                      ...
                      <td className="px-8 py-5">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-lg bg-gray-100 dark:bg-slate-800 flex items-center justify-center text-xs font-bold text-gray-500 dark:text-slate-400">
                            {f.client?.charAt(0).toUpperCase() || "?"}
                          </div>
                          <span className="text-sm font-bold text-gray-700 dark:text-slate-200 uppercase tracking-tight">
                            {f.client === "Commande non liée" || f.client === "Client inconnu" || f.client === "N/A" || f.client === "Aucun client" || f.client === t("Unlinked_Order") || f.client === t("Unknown_Customer") || f.client === t("No_Customer")
                              ? <span className="text-gray-400 italic font-medium lowercase tracking-normal">{f.client === "Commande non liée" ? t("Unlinked_Order") : f.client === "Client inconnu" ? t("Unknown_Customer") : f.client === "Aucun client" ? t("No_Customer") : f.client}</span>
                              : f.client}
                          </span>
                        </div>
                      </td>
